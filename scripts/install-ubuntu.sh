...
if [ -f "$ENV_FILE" ]; then
  ...
  if [ "$NON_INTERACTIVE" = false ]; then
    ...
    # BookStack 用 APP_URL 拼静态资源绝对 URL；用户浏览器必须能访问到
    if [ -z "${BOOKSTACK_APP_URL:-}" ]; then
      DETECTED_IP=$(hostname -I 2>/dev/null | awk '{print $1}')
      DEFAULT_URL="http://${DETECTED_IP:-localhost}:16875"
      read -rp "BookStack 公网 URL（回车默认 $DEFAULT_URL）: " BOOKSTACK_APP_URL
      [ -z "$BOOKSTACK_APP_URL" ] && BOOKSTACK_APP_URL="$DEFAULT_URL"
    fi
  fi
  [ -z "${EMBEDDING_API_KEY:-}" ] && { err "EMBEDDING_API_KEY 必填"; exit 1; }
  [ -z "${LLM_API_KEY:-}" ] && { err "LLM_API_KEY 必填"; exit 1; }
  [ -z "${BOOKSTACK_APP_URL:-}" ] && BOOKSTACK_APP_URL="http://localhost:16875"
  ...
  # BookStack APP_URL 写到 infra/.env（compose 替换 ${BOOKSTACK_APP_URL:-}）
  INFRA_ENV="$INSTALL_DIR/infra/.env"
  if [ ! -f "$INFRA_ENV" ] || ! grep -q '^BOOKSTACK_APP_URL=' "$INFRA_ENV"; then
    echo "BOOKSTACK_APP_URL=$BOOKSTACK_APP_URL" >> "$INFRA_ENV"
  else
    sed -i "s|^BOOKSTACK_APP_URL=.*|BOOKSTACK_APP_URL=$BOOKSTACK_APP_URL|" "$INFRA_ENV"
  fi
  ok "BOOKSTACK_APP_URL 写入 infra/.env: $BOOKSTACK_APP_URL"
...
echo ""
echo "  访问地址（新端口规划）："
echo "    🟣 知识中台 Web 控制台: http://${SERVER_IP}:15173   ← 你的紫色 UI"
echo "    📚 BookStack Wiki:      http://${SERVER_IP}:16875   ← 可选 Wiki UI"
echo "    🔌 qa-service API:      http://${SERVER_IP}:13001"
echo "    🔧 mcp-service:         http://${SERVER_IP}:13002/mcp（如启用）"
echo ""
